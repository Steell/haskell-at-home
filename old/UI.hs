...
draw (Overview (TreeView _deviceTree)) = overviewUi devices
 where
  ...
  label z = pack (replicate (generation z) ' ') <> (_dName . snd $ extract z)
...
smarthomeApp = App
 ...
 where
  ...
  event (Overview v) (VtyEvent (EvKey (KChar 'v') [MCtrl])) =
    continue . Overview $ case v of
      ListView l -> TreeView (toTree l)
  ...
  event (Overview v) (AppEvent (DeviceList dmap)) =
   ...
   where
    ...
    updateTree t dmap = let selected = t ^. T.treeSelectedL in _
    updateList l dmap = _
