...
data Notification
  ...
  | ValueChanged !NodeId !ValueId !ValueData
  | AwakeNodesQueried
  | AllNodesQueried
...
registerNotificationEvent m =
  ...
  where
    ...
    addHandler l = do
        ...
        return $ do
            unlessM (Z.manager_RemoveWatcher m cb Ptr.nullPtr) $
                putStrLn "could not remove notification listener"
            ...
            Z.ValueType_List -> do
                ...
                l <- alloca $
                         \p -> do
                             ...
                             decode p
                VTList l <$> (zip <$> (toContents valuesP)
                                  <*> (toContents labelsP))
            Z.ValueType_Schedule -> return VTSchedule
            Z.ValueType_Short -> alloca $
                \p -> do
